...
AngularApp.controller('Ctrl', function ($scope) {
    ...
    $scope.GetEmpList = function () {
        ...
        $.ajax({
            ...
            url: "../Emp/GetEmpList",
            dataType: 'Json',
            data: 'FulltimeFlag=0&company=' + $scope.UserFreeStyle.SearchCompany + "&DepartMentNo=" + $scope.UserFreeStyle.SearchDepartMentNo + "&status=" + $scope.UserFreeStyle.SearchEmpStatus + "&Sex=",
            async: false,
    ...
    $scope.update("../TempEmp/GridViewer", UserFreeStyle.PageNum);
    $scope.GetEmpList();
    ...
    $scope.Orderby = function (id, OrderField) {
        ...
        $.ajax({
            type: "POST",
            url: "../TempEmp/SaveUserFreeStyle",
            dataType: 'Json',
            data: 'UserFreeStyle=' + angular.toJson($scope.UserFreeStyle),
            async: false,
            success: function (data) {

                $scope.update("../TempEmp/GridViewer", 0);
    ...
    $scope.DelBtn = function () {
        $.ajax({
            type: "POST",
            url: "../TempEmp/TempEmpFormDel",
            dataType: 'Json',
            data: 'TempEmpID=' + $scope.DeleteRecordID,
            async: false,
            success: function (data) {
                document.location.href = "../TempEmp/";
            },
            error: function () {
                alert("Error !");
            }
    ...
    $scope.Link = function (RecordID) {
        ...
        $.ajax({
            ...
            async: false,
            success: function (data) {
                document.location.href = "../TempEmp/TempEmpForm";
            },
            error: function () {
                alert("Error !");
            }
